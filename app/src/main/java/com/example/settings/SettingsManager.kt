...
class SettingsManager(private val context: Context) {
    ...
    val geminiPrompt: Flow<String> = context.dataStore.data.map { it[GEMINI_PROMPT] ?: """
        ...
        ملاحظة هامة جداً: يجب أن تكون جميع البيانات المستخرجة (مثل اسم القارئ، العنوان، التصنيف) باللغة العربية فقط حصراً. إذا كان الاسم بالإنجليزية، قم بترجمته للعربية (مثل: Nasser Al-Qatami يصبح ناصر القطامي).
        ...
        [RECITER]هنا اسم القارئ او الشيخ بدقة باللغة العربية[/RECITER]
        [START]هنا رقم آية البداية (رقم فقط)[/START]
        [END]هنا رقم الاية النهائية (رقم فقط)[/END]
        [TITLE]هنا العنوان المناسب للمقطع باللغة العربية[/TITLE]
        [CATEGORY]هنا التصنيف باللغة العربية (طمأنينة، خشوع، سكينة، دعاء)[/CATEGORY]
        ...
        تأكد من عدم إضافة مسافات إضافية داخل الأقواس. أعد الرد باستخدام هذه الرموز فقط وبدون أي تنسيقات JSON.
    """.trimIndent() }
